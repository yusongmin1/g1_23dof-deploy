...
class Config_23:
    def __init__(self, file_path) -> None:
        with open(file_path, "r") as f:
            ...
            self.joint_names_lab= config["joint_names_lab"]
            self.joint_names_real = config["joint_names_real"]
            self.default_angles_lab_policy= np.array(config["default_angles_lab_policy"], dtype=np.float32)
            self.default_angles_real= np.array(config["default_angles_real"], dtype=np.float32)
